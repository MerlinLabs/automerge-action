...
async function handleCheckUpdate(context, eventName, event) {
  ...
  if (action !== 'completed') {
    logger.info("A status check is not yet complete:", eventName, action);
    throw new NeutralExitError();
  } else {
    if (payload.conclusion === 'success') {
      ...
      await updateAndMergePullRequest(context, pull_request);
    } else {
      logger.info("A status check completed unsuccessfully:", eventName, payload.conclusion);
      throw new NeutralExitError();
    }
  }
}
